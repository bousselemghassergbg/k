...
import { Plus, Search, Users, DollarSign, Trophy, Calendar, LogIn, Eye, Settings, Copy, Check, Crown, Medal, Award } from 'lucide-react';
...
export default function UserLeaguesManager() {
  ...
  const [overallRankings, setOverallRankings] = useState<LeagueParticipant[]>([]);
  ...
  const [activeTab, setActiveTab] = useState<'overall' | 'my-leagues' | 'joined-leagues'>('overall');
    ...
    try {
      ...
      if (fantasyTeams && fantasyTeams.length > 0) {
        ...
        const { data: leagues, error: leaguesError } = await supabase
          ...
          .neq('name', 'Overall') // Exclude Overall league
          .order('created_at', { ascending: false });
  ...
  const fetchOverallLeague = async () => {
    ...
    try {
      ...
      setOverallLeague(overallData);

      // Get Overall league rankings
      const { data: rankings, error: rankingsError } = await supabase
        ...
        `)
        .eq('league_id', overallData.league_id)
      ...
      console.error('Error fetching overall league:', error);
    }
  };
  ...
  const filteredOverallRankings = overallRankings.filter(participant =>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        ...
        <p className="text-gray-600">
          View overall rankings, create your own leagues, or join existing ones to compete with other players.
        </p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm overflow-hidden">
        <div className="border-b border-gray-200">
          <nav className="flex space-x-8 px-6">
            ...
            >
              Overall Rankings ({overallRankings.length})
            </button>
            <button
        ...
        {activeTab === 'overall' && (
          <div className="p-6">
            <div className="mb-4">
              <h2 className="text-xl font-semibold text-gray-900 mb-2">Overall League Rankings</h2>
              <p className="text-gray-600">Global leaderboard showing all fantasy teams</p>
            </div>
            
            {filteredOverallRankings.length === 0 ? (
              <div className="text-center py-12">
                <Trophy className="h-12 w-12 text-gray-400 mx-auto mb-4" />
                <h3 className="text-lg font-medium text-gray-900 mb-2">No rankings available</h3>
                <p className="text-gray-500">Rankings will appear once gameweeks are completed.</p>
              </div>
            ) : (
              <div className="space-y-2">
                {filteredOverallRankings.map((participant) => (
                  <div key={participant.fantasy_team_id} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-gray-900 text-lg">{participant.total_points} pts</div>
                    </div>
                  </div>
                ))}
              </div>
