...
import { Users, DollarSign, Trophy, Calendar, Edit, Save, X, Plus, Minus, Crown, Star, TrendingUp, Clock, AlertTriangle } from 'lucide-react';
...
export default function MyTeam() {
  ...
  const getNextDeadline = () => {
    const relevantGameweek = gameweekStatus.current || gameweekStatus.next;
    if (!relevantGameweek) return null;
  ...
  const getCurrentGameweekInfo = () => {
    if (gameweekStatus.isGameweekActive && gameweekStatus.current) {
      return `Gameweek ${gameweekStatus.current.gameweek_number} is currently active`;
    } else if (gameweekStatus.next) {
      return `Next: Gameweek ${gameweekStatus.next.gameweek_number}`;
    } else if (gameweekStatus.current) {
      return `Current: Gameweek ${gameweekStatus.current.gameweek_number}`;
    }
          ...
          <div className="bg-white/20 backdrop-blur-sm p-4 rounded-xl border border-white/20">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">{roster.length}/15</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="flex items-center text-gray-700">
              ...
              <span className="font-medium">{getCurrentGameweekInfo()}</span>
            </div>
            {nextDeadline && (
              <div className="flex items-center text-gray-600">
                <AlertTriangle className="h-4 w-4 mr-2 text-orange-500" />
                <span className="text-sm">
                  Next deadline: {nextDeadline.toLocaleDateString()} at {nextDeadline.toLocaleTimeString()}
                </span>
              </div>
            )}
          </div>
              ...
              <div className="flex flex-col items-center">
                ...
                {editMode && (
                  <button
                    ...
                    disabled={!transferData.canMakeTransfers}
    ...
    <div className="relative flex flex-col items-center">
      ...
      {editMode && (
        <div className="mt-2 flex space-x-1">
          <button
            onClick={onReplace}
            disabled={!canMakeTransfers}
            className={`px-2 py-1 rounded-lg text-xs transition-colors shadow-md ${
              canMakeTransfers 
